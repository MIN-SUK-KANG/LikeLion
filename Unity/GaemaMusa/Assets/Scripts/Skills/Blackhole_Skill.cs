...
public class Blackhole_Skill : Skill
{
    ...
    public override bool CanUseSkill() // 스킬 사용 가능 여부 체크
    {
        return base.CanUseSkill();
    }    
    public override void UseSkill() // 스킬 사용 메서드
    {
        base.UseSkill();        
        GameObject newBlackHole = Instantiate(blackHolePrefab, player.transform.position, Quaternion.identity);                 // 새 블랙홀 생성
        currentBalckhole = newBlackHole.GetComponent<Blackhole_Skill_Controller>();                                             // 블랙홀 컨트롤러 설정
        currentBalckhole.SetupBlackhole(maxSize, growSpeed, shrinkSpeed, amountOfAttacks, cloneCooldown, blackholeDuration);    // 블랙홀 설정 (크기, 속도, 공격수, 쿨다운, 지속시간)
    }

    public bool BlackholeFinished() // 블랙홀 종료 여부 확인 함수
    {
        if (!currentBalckhole)
            return false;
